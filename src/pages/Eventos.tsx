...
import { Plus, Clock, MapPin, Users, Edit2, Trash2, Loader2, Calendar, RefreshCw, Unplug } from "lucide-react";
...
const Eventos = () => {
  ...
  const [view, setView] = useState<"lista" | "passados" | "calendario" | "google">("lista");
  ...
  // Fetch Google Calendar events when switching to that view
  useEffect(() => {
    if (view === "google" && gcal.connected) {
      const now = new Date();
      const threeMonthsAgo = new Date(now);
      threeMonthsAgo.setMonth(now.getMonth() - 3);
      const sixMonthsAhead = new Date(now);
      sixMonthsAhead.setMonth(now.getMonth() + 6);
      gcal.fetchEvents(threeMonthsAgo.toISOString(), sixMonthsAhead.toISOString()).then(setGoogleEvents);
    }
  }, [view, gcal.connected]);
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-lg border border-border overflow-hidden">
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${view === "google" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}
              >
                📅 Google
              </button>
            </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => {
                      ...
                      const future = new Date(now); future.setMonth(now.getMonth() + 6);
