...
import {
  ...
  Heart, Activity, HandHeart, Briefcase, Home, Trophy, HelpCircle, Headphones, GitMerge,
} from "lucide-react";
...
// Coordenações operacionais genéricas (para qualquer gabinete parlamentar)
const coordenacaoItems = [
  { icon: Headphones, label: "Atendimento SAC",     path: "/coordenacao/atendimento" },
  ...
  { icon: GitMerge,   label: "Articulação Política", path: "/coordenacao/articulacao" },
  { icon: Gavel,      label: "Legislativo",          path: "/coordenacao/legislativo" },
];

// Setores SAC conforme especificação técnica
const movimentoSetores = [
  { icon: Heart,       label: "Saúde",              path: "/movimentos/saude" },
  { icon: Activity,    label: "TEA",                path: "/movimentos/tea" },
  { icon: HandHeart,   label: "Assistência Social", path: "/movimentos/assistencia-social" },
  { icon: Building2,   label: "Infraestrutura",     path: "/movimentos/infraestrutura" },
  { icon: Briefcase,   label: "Empreendedorismo",   path: "/movimentos/empreendedorismo" },
  { icon: Home,        label: "Habitação",          path: "/movimentos/habitacao" },
  { icon: Trophy,      label: "Esporte e Lazer",    path: "/movimentos/esporte-lazer" },
  { icon: HelpCircle,  label: "Outros",             path: "/movimentos/outros" },
];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 py-2 px-2 overflow-y-auto scrollbar-thin">
        ...
        <div className="space-y-0.5">
          {legislativoItems.filter(i => i.path !== "/movimentos").map((item) => renderLink(item))}

          {/* Movimentos com sub-setores SAC */}
          <Link
            to="/movimentos"
            onClick={onNavigate}
            className={cn(
              "flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 w-full cursor-pointer relative",
              location.pathname.startsWith("/movimentos")
                ? "bg-sidebar-accent text-sidebar-primary font-semibold nav-active-indicator"
                : "text-sidebar-foreground/70 hover:bg-sidebar-accent/40 hover:text-sidebar-foreground"
            )}
          >
            ...
            <ClipboardList className={cn("w-4 h-4 shrink-0", location.pathname.startsWith("/movimentos") && "text-sidebar-primary")} />
            {showLabels && (
              <>
                <span className="truncate flex-1 text-left text-xs font-medium">Movimentos</span>
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); setMovOpen(!movOpen); }}
                  className="p-0.5 rounded hover:bg-sidebar-accent/80"
                >
                  <ChevronDown className={cn("w-3.5 h-3.5 transition-transform text-sidebar-foreground/50", movOpen && "rotate-180")} />
                </button>
              </>
            )}
          </Link>
          {movOpen && showLabels && (
            <div className="mt-0.5 space-y-0.5">
              {movimentoSetores.map((item) => renderLink(item as NavItem, true))}
            </div>
          )}
        </div>
