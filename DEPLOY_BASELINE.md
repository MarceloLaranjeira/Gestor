...
Toda nova mudança deve partir deste baseline:

1. Abrir somente a pasta `Gestor-main`.
2. Criar alteração em cima de `master` (que está espelhada em `main`).
3. Publicar e validar produção.
4. Manter `main` e `master` no mesmo commit após cada release.
